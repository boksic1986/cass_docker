...
#		next if $win_line[0]>22;
		...
		if($tags==0){print STDERR "Null window:$win_line[0]:$win_line[1]-$win_line[2]\n";}
		my $gc_spe=$tags==0?"$win_line[3]":$gc/$length;
		my $lines="$win_line[0]\t$win_line[1]\t$win_line[2]\t$win_line[3]\t$gc_spe\t$tags";
		push @lines,$lines;
	...
	my ($Y_num,$Y_copyratio)=(0,0);
	foreach my $l(@lines){
		print OUT "$l\t",(split /\s+/,$l)[-1]/$auto_mean,"\t",((split /\s+/,$l)[-1]*$auto_mean/$box_mean{sprintf("%.2f",(split /\s+/,$l)[-3])}{sprintf("%.2f",(split /\s+/,$l)[-2])})/$cor_sum,"\n";
#		print OUT2 "$l\t",(split /\s+/,$l)[-1]/$auto_mean*2,"\t",((split /\s+/,$l)[-1]*$auto_mean/$box_mean{sprintf("%.2f",(split /\s+/,$l)[-3])}{sprintf("%.2f",(split /\s+/,$l)[-2])})/$cor_sum*2,"\n";
		if((split /\s+/,$l)[0]==24){
			$Y_num++;
			$Y_copyratio+=((split /\s+/,$l)[-1]*$auto_mean/$box_mean{sprintf("%.2f",(split /\s+/,$l)[-3])}{sprintf("%.2f",(split /\s+/,$l)[-2])})/$cor_sum;
		}
	}
	$Y_copyratio/=$Y_num;
	print $Y_copyratio>0.25?"M":"F";
	close IN;
	close OUT;
#	close OUT2;
	close WIN;
